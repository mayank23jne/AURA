...
def get_audit_date(report: dict) -> str:
    # 1. Try top-level timestamp first
    ts = report.get("timestamp")
    if ts:
        return ts[:19]

    # 2. Fallback: get timestamp from test results
    test_results = (
        report.get("audit_data", {})
              .get("results", {})
              .get("test_results", [])
    )
    ...
    if timestamps:
        # pick earliest test execution time
...
def generate_report_pdf(report: dict) -> bytes:
    ...
    print('date', report.get('timestamp'))
    # Executive summary table
    audit_date = get_audit_date(report)
    ...
    exec_data = [
        ...
        ['Date:', audit_date, 'Status:', report.get('status', 'Unknown').upper()],
